...
/** Durable index: maps runId → event, survives flush for outcome correlation */
const pendingOutcomes = new Map<string, PreflightEvent>();
...
export function recordPreflight(result: PreflightResult): string {
  ...
  pendingOutcomes.set(event.runId, event);
...
export function recordDeploymentOutcome(
  ...
  // Check the durable index first — the event may have already been flushed
  const event = pendingOutcomes.get(runId);
  if (!event) return;
  ...
  // If the event was already flushed from the buffer, re-queue it so the
  // outcome update reaches the sink
  if (!eventBuffer.includes(event)) {
    eventBuffer.push(event);
  }
}
