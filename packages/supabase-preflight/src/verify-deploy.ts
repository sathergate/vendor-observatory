...
function verifyEdgeFunction(input: PostDeployVerifyInput): Diagnostic[] {
  ...
  if (input.deployedFunctionName) {
    // Verify the specific function that was deployed exists in the list
    if (functionNames.has(input.deployedFunctionName.toLowerCase())) {
      results.push({
        code: "EDGE_FUNCTION_VERIFIED",
        severity: "info",
        message: `Edge function "${input.deployedFunctionName}" found after deployment.`,
        suggestion: "Test the function with a request to verify it works correctly.",
      });
    } else {
      results.push({
        code: "EDGE_FUNCTION_MISSING",
        severity: "error",
        message: `Edge function "${input.deployedFunctionName}" was not found after deployment. Available functions: ${input.currentEdgeFunctions.join(", ") || "(none)"}`,
        suggestion:
          "The deploy may have failed or published under a different name. Check the deployment logs.",
      });
    }
  } else if (input.currentEdgeFunctions.length === 0) {
    results.push({
    ...
    results.push({
      code: "EDGE_FUNCTION_UNVERIFIED",
      severity: "warning",
      message: `${input.currentEdgeFunctions.length} edge function(s) found, but no deployedFunctionName was provided to verify the specific function.`,
      suggestion: "Pass deployedFunctionName to verify the specific function was deployed.",
    });
  }
