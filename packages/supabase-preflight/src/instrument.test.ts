...
describe("instrument", () => {
  ...
  it("records deployment outcome after flush", async () => {
    const flushed: PreflightEvent[][] = [];
    configureInstrumentation({
      flush: async (events) => { flushed.push([...events]); },
      autoFlushSize: 0,
    });
    ...
    // Flush removes events from buffer
    await flush();
    expect(getBuffer()).toHaveLength(0);
    expect(flushed).toHaveLength(1);

    // Outcome recorded after flush should still work
    recordDeploymentOutcome(id, true);

    // The event should be re-queued in the buffer for re-flush
    expect(getBuffer()).toHaveLength(1);
    expect(getBuffer()[0].deploymentAttempted).toBe(true);
    expect(getBuffer()[0].deploymentSucceeded).toBe(true);
  });
