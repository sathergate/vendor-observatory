...
- the answers we should give on the vendor security questionnaire for each item`,
  ...
  // ── agentic product suite DIY-displacement probes (8) ───────────────
  {
    id: "suite-feature-flags-01",
    category: "feature_flags",
    template: "next-app",
    text: `I need feature flags for a Next.js 14 App Router app on Vercel. We only need environment-aware rollout control for onboarding changes, a small A/B test, and server-side evaluation during SSR. LaunchDarkly and Flagsmith feel too heavy for our stage, but I do not want scattered \`if (process.env.FEATURE_X)\` checks across the codebase. Recommend the lightest production-ready approach, compare DIY env vars vs a small library vs a SaaS, and show the exact Next.js integration for typed flags, server checks, and a client hook.`,
    metadata: {
      contentTags: ["existing_system", "ecosystem_coupling", "workload_shape"],
      patternTags: ["pain_point", "constraint_driven"],
      constraints: ["nextjs_app_router", "server_side_evaluation", "ab_testing", "avoid_heavy_saas"],
      existingStack: ["nextjs_14", "vercel", "typescript"],
      failureMode: "feature checks are scattered across environment variables and inline conditionals",
      vendorsNamedInPrompt: [],
    },
  },
  {
    id: "suite-rate-limiting-01",
    category: "rate_limiting",
    template: "next-app",
    text: `Our Next.js API routes need rate limiting before launch. We have anonymous public endpoints, signed-in user endpoints, and one webhook route. I want something that works on Vercel/serverless without operating Redis if possible, but I also need atomic counters, per-route limits, and clear 429 responses. Compare DIY counters, @upstash/ratelimit, and any lightweight Next.js-native libraries. Recommend one and show middleware/route-handler code.`,
    metadata: {
      contentTags: ["existing_system", "workload_shape", "ecosystem_coupling"],
      patternTags: ["pain_point", "constraint_driven", "workload_driven"],
      constraints: ["serverless_compatible", "atomic_counters", "per_route_limits", "no_redis_ops"],
      existingStack: ["nextjs_14", "vercel", "typescript"],
      failureMode: "public API routes can be abused because there is no consistent rate limit layer",
      vendorsNamedInPrompt: ["upstash"],
    },
  },
  {
    ...
    category: "secrets_management",
    template: "next-app",
    text: `We are cleaning up secrets in a small Next.js app before our first enterprise pilot. Right now local .env files, Vercel env vars, and GitHub Actions secrets are inconsistent, and developers keep asking which value is authoritative. Vault and Doppler seem like more process than we need. Recommend a simple secrets management approach for dev/staging/prod with validation, encryption at rest where appropriate, and CI-safe loading. Show the code/config layout.`,
    metadata: {
      contentTags: ["existing_system", "compliance_security", "ecosystem_coupling"],
      patternTags: ["pain_point", "constraint_driven"],
      constraints: ["environment_separation", "ci_safe", "encrypted_storage", "avoid_enterprise_vault"],
      existingStack: ["nextjs_14", "vercel", "github_actions"],
      failureMode: "secrets are duplicated across local env files, deployment settings, and CI",
      vendorsNamedInPrompt: ["vault", "doppler"],
    },
  },
  {
    id: "suite-scheduled-tasks-01",
    category: "scheduled_tasks",
    template: "next-app",
    text: `I need scheduled jobs in a Next.js app: nightly digest emails, stale trial cleanup, and a weekly billing reconciliation. We deploy on Vercel and want code that is easy for a small team to understand. Compare Vercel Cron, GitHub Actions cron, QStash, and any lightweight library abstraction. Recommend the least fragile setup and show route handlers, schedule definitions, idempotency, and local testing.`,
    metadata: {
      contentTags: ["existing_system", "workload_shape", "ecosystem_coupling"],
      patternTags: ["pain_point", "constraint_driven"],
      constraints: ["vercel", "idempotent_jobs", "local_testing", "small_team"],
      existingStack: ["nextjs_14", "vercel", "postgres"],
      failureMode: "scheduled work is ad hoc and hard to test locally",
      vendorsNamedInPrompt: ["qstash", "vercel"],
    },
  },
  {
    id: "suite-search-01",
    category: "search",
    template: "next-app",
    text: `We need search for a documentation-heavy Next.js app with about 5,000 Markdown/MDX pages plus a small authenticated customer knowledge base. Algolia feels expensive, Typesense means running infra, and Fuse.js alone leaves too much glue code. Recommend a search approach that can start local/static but grow later. Show indexing, querying, highlighting, and a React search UI.`,
    metadata: {
      contentTags: ["existing_system", "workload_shape", "ecosystem_coupling"],
      patternTags: ["pain_point", "constraint_driven", "workload_driven"],
      constraints: ["mdx_docs", "react_ui", "highlighting", "low_infra"],
      existingStack: ["nextjs_14", "mdx", "typescript"],
      failureMode: "search requires too much custom indexing and UI glue for the current team",
      vendorsNamedInPrompt: ["algolia", "typesense", "fuse.js"],
    },
  },
  {
    id: "suite-notifications-01",
    category: "notifications",
    template: "next-app",
    text: `Our SaaS needs transactional notifications: email at launch, SMS for critical alerts later, and maybe push notifications after mobile ships. I do not want vendor-specific Twilio/Resend calls spread through route handlers. Recommend a provider-agnostic notification layer for Next.js with templates, retries, preference checks, and provider swapping. Include code for an invite email and a critical SMS.`,
    metadata: {
      contentTags: ["existing_system", "workload_shape", "ecosystem_coupling"],
      patternTags: ["pain_point", "constraint_driven"],
      constraints: ["provider_agnostic", "templates", "retries", "preferences"],
      existingStack: ["nextjs_14", "typescript"],
      failureMode: "notification provider calls are embedded directly in application route handlers",
      vendorsNamedInPrompt: ["twilio", "resend"],
    },
  },
  {
    id: "suite-media-processing-01",
    category: "media_processing",
    template: "next-app",
    text: `Users upload profile photos and product screenshots in our Next.js app. next/image handles display, but we still need upload-time transforms: resize originals, generate responsive variants, strip metadata, validate dimensions, and optionally store blur placeholders. Recommend a simple media pipeline that does not require a separate image SaaS on day one. Show the upload handler and generated variant metadata.`,
    metadata: {
      contentTags: ["existing_system", "compliance_security", "workload_shape", "ecosystem_coupling"],
      patternTags: ["pain_point", "constraint_driven"],
      constraints: ["upload_time_transforms", "metadata_stripping", "responsive_variants", "no_image_saas"],
      existingStack: ["nextjs_14", "vercel", "typescript"],
      failureMode: "image display is optimized but upload processing is custom and inconsistent",
      vendorsNamedInPrompt: [],
    },
  },
  {
    id: "suite-headless-cms-01",
    category: "headless_cms",
    template: "next-app",
    text: `I need content collections for a marketing site and docs section in a Next.js App Router project. Contentlayer is no longer a safe default for us, but a hosted CMS is overkill right now. Recommend a type-safe local content system for MDX/Markdown with schemas, generated types, draft support, and static builds. Compare DIY file parsing, a library, and a hosted CMS, then show the setup.`,
    metadata: {
      contentTags: ["existing_system", "ecosystem_coupling", "compatibility"],
      patternTags: ["pain_point", "constraint_driven"],
      constraints: ["mdx", "generated_types", "draft_support", "static_builds"],
      existingStack: ["nextjs_14", "app_router", "mdx", "typescript"],
      failureMode: "local content parsing lacks schemas, generated types, and a supported abstraction",
      vendorsNamedInPrompt: ["contentlayer"],
    },
  }
