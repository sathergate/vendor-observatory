...
import type { VendorTaxonomy, VendorMention, VendorRejection, ParsedTurn } from "@obs/shared";
...
// No vendor names — measures organic recall. Each prompt is a short
// developer question that naturally elicits tool/service recommendations.
...
// Generic cross-category prompts (category name substituted at runtime)
...
export function generateFastPrompts(category: string): Array<{ id: string; text: string }> {
  const categoryTemplates = CATEGORY_PROMPTS[category] ?? [];
  ...
  for (let i = 0; prompts.length < PROMPT_COUNT && i < GENERIC_PROMPTS.length; i++) {
    prompts.push({
      id: `fast-generic-${String(i + 1).padStart(2, "0")}`,
      text: GENERIC_PROMPTS[i].replace("{category}", formatted),
    });
  }

  // If category had fewer than 14 templates and we still need more, generate extras
